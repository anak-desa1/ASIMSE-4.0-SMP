...
class Laporan_model extends CI_Model
{
    ...
    public function arsip($tgl_awal, $tgl_akhir, $id_ruangan, $id_lemari, $id_rak, $id_box, $id_map, $id_urut, $jenis_dokumen)
    {
        if ($id_ruangan != "" && $id_ruangan != "all") {
            $param = "AND mst_dokumen.id_ruangan = '$id_ruangan'";
        } else {
            $param =  '';
        }

        if ($id_lemari != "" && $id_lemari != "all") {
            $param2 = "AND mst_dokumen.id_lemari = '$id_lemari'";
        } else {
            $param2 =  '';
        }

        if ($id_rak != "" && $id_rak != "all") {
            $param3 = "AND mst_dokumen.id_rak = '$id_rak'";
        } else {
            $param3 =  '';
        }

        if ($id_box != "" && $id_box != "all") {
            $param4 = "AND mst_dokumen.id_box = '$id_box'";
        } else {
            $param4 =  '';
        }

        if ($id_map != "" && $id_map != "all") {
            $param4 = "AND mst_dokumen.id_map = '$id_map'";
        } else {
            $param4 =  '';
        }

        if ($id_urut != "" && $id_urut != "all") {
            $param5 = "AND mst_dokumen.id_urut = '$id_urut'";
        } else {
            $param5 =  '';
        }

        if ($jenis_dokumen != "" && $jenis_dokumen != "all") {
            $param6 = "AND mst_dokumen.id_urut = '$jenis_dokumen'";
        } else {
            $param6 =  '';
        }

        $q = $this->db->query("SELECT * FROM mst_dokumen 
                            INNER JOIN mst_jenis_dokumen ON mst_dokumen.id_jenis_dokumen = mst_jenis_dokumen.id_jenis_dokumen 
									...
									INNER JOIN mst_urut ON mst_dokumen.id_urut = mst_urut.id_urut WHERE tanggal_dokumen >= '$tgl_awal' AND tanggal_dokumen <= '$tgl_akhir' $param $param2 $param3 $param4 $param5 $param6 ORDER BY tanggal_dokumen DESC");
        return $q;
    }
}
