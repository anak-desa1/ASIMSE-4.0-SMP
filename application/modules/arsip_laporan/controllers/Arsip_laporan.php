...
class Arsip_laporan extends CI_Controller
{
	...
	public function proses_tampil_arsip()
	{
		...
		redirect("arsip_laporan/arsip/" . $tgl_awal . "/" . $tgl_akhir . "/" . $id_ruangan . '/' . $id_lemari . '/' . $id_rak . '/' . $id_box . '/' . $id_map . '/' . $id_urut . '/' . $jenis_dokumen);
	}
	...
	public function arsip($tgl_awal = "", $tgl_akhir = "", $id_ruangan = "", $id_lemari = "", $id_rak = "", $id_box = "", $id_map = "", $id_urut = "", $jenis_dokumen = "")
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if (!empty($tgl_awal) && !empty($tgl_akhir)) {
				...
				$data['laporan_arsip'] = $this->Laporan_model->arsip($tgl_awal, $tgl_akhir, $id_ruangan, $id_lemari, $id_rak, $id_box, $id_map, $id_urut, $jenis_dokumen);
			} else {
				$data['laporan_arsip'] = "";
