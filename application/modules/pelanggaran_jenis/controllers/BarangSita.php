...
class BarangSita extends CI_Controller
{
	...
	{
		$cek = $this->db->query("SELECT id_barangsita_siswa FROM mst_barangsita_siswa WHERE id_barangsita_siswa = '$id_barangsita_siswa'");
		if ($cek->num_rows() > 0) {
			$this->benchmark->mark('code_start');
			$data['title'] = 'Barang Sitaan Siswa';
			$data['home'] = 'Jenis Pelanggaran';
			...
			$data['judul2'] = "Ubah";
			$data['tipe'] = 'edit';
			$get = $this->db->query("SELECT id_barangsita_siswa,mst_barangsita_siswa.id_siswa, nama_kelas, nama_siswa, tanggal_sita, keterangan_sita FROM barangsita_siswa 
										INNER JOIN mst_siswa ON mst_barangsita_siswa.id_siswa = mst_siswa.id_siswa 
										INNER JOIN mst_kelas ON mst_barangsita_siswa.id_kelas = mst_kelas.id_kelas WHERE id_barangsita_siswa = '$id_barangsita_siswa'");
			$d = $get->row();
	...
	public function barangsita_save()
	{
		...
		if ($tipe == "add") {
			...
			$this->db->insert("mst_barangsita_siswa", $in);
			...
			$this->db->update("mst_barangsita_siswa", $in, $where);
	...
	public function barangsita_hapus($id)
	{
		...
		$this->db->delete("mst_barangsita_siswa", $where);
