...
if (!function_exists('ppdb_aktif_login')) {
    function ppdb_aktif_login()
    {
        $ci = get_instance();
...
if (!function_exists('ppdb_akses_user')) {
    function ppdb_akses_user()
    {
        $ci = get_instance();
        $cek = $ci->db->select('*')
            ->from('ppdb_user_access_menu a')
            ->join('ppdb_user_sub_menu m', 'm.menu_id = a.menu_id', 'left')
