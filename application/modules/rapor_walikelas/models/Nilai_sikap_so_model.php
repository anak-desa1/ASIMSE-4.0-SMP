...

{
    ...
    public function get_siswa_pts()
    {
        ...
        $result = $this->db->get_where('t_nilai_sikap_so', ["penilaian" => 'PTS', 'is_wali' == 'Y', 'tasm' ==  $tahun])->row_array();
        if ($result['predikat'] == '' || $result['selalu']  ==  0 || $result['mulai_meningkat'] == 0) {
            $bagidata =
                $this->db->select('s.nama,s.nis,na.selalu,na.mulai_meningkat,w.id_guru')
                ->from('t_kelas_siswa k')
                ->join('m_siswa s', 'k.id_siswa = s.nis', 'left')
                ->join('t_walikelas w', 'k.rombel = w.id_kelas', 'left')
    ...
    public function get_siswa_pas()
    {
        ...
        $result = $this->db->get_where('t_nilai_sikap_so', ["penilaian" => 'PAS', 'is_wali' == 'Y', 'tasm' ==  $tahun])->row_array();
        if ($result['predikat'] == '' || $result['selalu']  ==  0 || $result['mulai_meningkat'] == 0) {
            $bagidata =
    ...
    {
        $bagidata =
            $this->db->select('k.*,c.*,sh.*,ks.tingkat,s.nama,w.id_guru,na.*,s.nis')
            ->from('t_kelas_siswa k')
