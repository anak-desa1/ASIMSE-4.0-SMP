...
class Peminjaman extends CI_Controller
{
	...
	{
		$cek = $this->db->query("SELECT id_peminjaman_siswa FROM mst_peminjaman_siswa WHERE id_peminjaman_siswa = '$id_peminjaman_siswa'");
		if ($cek->num_rows() > 0) {
			$this->benchmark->mark('code_start');
			$data['title'] = 'Peminjaman Siswa';
			$data['home'] = 'Jenis Pelanggaran';
			...
			$data['judul2'] = "Ubah";
			$data['tipe'] = 'edit';
			$get = $this->db->query("SELECT id_peminjaman_siswa,mst_peminjaman_siswa.id_siswa, nama_kelas, nama_siswa, tanggal_pinjam, keterangan_pinjam FROM mst_peminjaman_siswa 
										INNER JOIN m_siswa ON mst_peminjaman_siswa.id_siswa = m_siswa.id_siswa 
										INNER JOIN m_kelas ON mst_peminjaman_siswa.id_kelas = mst_kelas.id_kelas WHERE id_peminjaman_siswa = '$id_peminjaman_siswa'");
			$da = $get->row();
	...
	public function peminjaman_save()
	{
		...
		if ($tipe == "add") {
			$get_kelas = $this->db->query("SELECT id_kelas FROM m_siswa WHERE id_siswa = '$in[id_siswa]'")->row();
