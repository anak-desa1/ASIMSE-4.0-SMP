...
<script>
    $(document).ready(function() {
        $("#FormKI1").submit(function(e) {
            e.preventDefault();
            var id = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'tabel_ki1' ?>/" + id;
            $("#mapel").load(url);
        })
...
<script>
    $(document).ready(function() {
        $("#FormKI2").submit(function(e) {
            e.preventDefault();
            var id = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'tabel_ki2' ?>/" + id;
            $("#mapel").load(url);
        })
