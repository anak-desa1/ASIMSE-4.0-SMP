...
class Combo_model extends CI_Model {
	...
	public function combo_jenis_dokumen($id="") {
		...
		foreach($q->result() as $h) {
			if($id == $h->id_jenis_dokumen) {
				$hasil .= '<option value="'.$h->id_jenis_dokumen.'" selected="selected">'.$h->nama_jenis_dokumen.'</option>';
			} else {
				$hasil .= '<option value="'.$h->id_jenis_dokumen.'">'.$h->nama_jenis_dokumen.'</option>';
			}
		}
		return $hasil;
	}
