                        ...
                        foreach ($kuota as $kuota) :
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-sm btn-warning" title="Edit" id="edit_kuota" data-toggle="modal" data-target="#editModal" data-id_jurusan="<?= $kuota['id_jurusan']; ?>" data-nama_jurusan="<?= $kuota['nama_jurusan']; ?>" data-kuota="<?= $kuota['kuota']; ?>" data-status="<?= $kuota['status']; ?>"><i class="fa fa-edit"></i></a>
                                    <a href="<?= base_url() ?>ppdb_setting/deldata_kuota/<?= $kuota['id_jurusan'] ?>" class="btn btn-sm btn-danger btn-hapus"><i class="fa fa-trash-alt"></i></a>
                                </td>
                            </tr>
                        <?php endforeach ?>
                    </tbody>
            ...
            <form method="post" action="<?= base_url() . $this->uri->segment(1, 0) ?>/simpan_tambah_kuota" role="form" id="form-action" enctype="multipart/form-data">
                <div class="modal-body">                    
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jenis_daftar" id="jenis_daftar" class="selectpicker form-control" data-live-search="true" required>
                                <option value="">Pilih Jenis Daftar</option>
                                <?php foreach ($jenis_daftar as $rl) {
                                    echo '<option value="' . $rl['nama_jenis'] . '">' . $rl['nama_jenis'] . '</option>';
                                } ?>
                            </select>
                    </div>
                </div>
