...

{
    ...
    public function get_siswa_tampil()
    {
        ...
        $bagidata =
            $this->db->select('a.*, b.nis, b.nama, a.s, a.i, a.a')
    ...
    // end index
    // tambah
    // public function get_siswa_add_pts()
    // {
    //     $get_tasm = $this->db->get_where('t_tahun', ['aktif' => 'Y', 'kd_sekolah' => $this->session->userdata('kd_sekolah')])->row_array();
    //     $data['tasm'] = $get_tasm['tahun'];
    //     $tahun = substr($data['tasm'], 0, 4);
    ...
    //     $result = $this->db->get_where('t_nilai_absensi', ["penilaian" => 'PTS'])->row_array();
    //     if ($result > 1) {
    //         $bagidata =
    //             $this->db->select('a.*, b.nis, b.nama, d.*')
    //             ->from('t_nilai_absensi a')
    //             ->join('m_siswa b', 'a.id_siswa = b.nis', 'left')
    //             ->join('t_kelas_siswa d', 'a.id_siswa = d.id_siswa', 'left')
    //             ->join('t_walikelas e', 'd.rombel = e.id_kelas', 'left')
    ...
    public function get_siswa_add_pas()
    {
        $get_tasm = $this->db->get_where('t_tahun', ['aktif' => 'Y', 'kd_sekolah' => $this->session->userdata('kd_sekolah')])->row_array();
        $data['tasm'] = $get_tasm['tahun'];
        $tahun = substr($data['tasm'], 0, 4);
