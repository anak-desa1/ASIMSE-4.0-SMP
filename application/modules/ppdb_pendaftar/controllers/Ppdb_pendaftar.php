...
class Ppdb_pendaftar extends CI_Controller
{
    ...
    public function data_pendaftar()
    {
        ...
        $data['tahun'] = $this->psb->getDaftarTahun();
    ...
    function add_ajax_periode($tahun)
    {
        $query = $this->db->get_where('ppdb_periode', array('tahun' => $tahun));
        $data = "<option value=''>- Select Periode -</option>";
        foreach ($query->result() as $value) {
            $data .= "<option value='" . $value->periode . "'>" . $value->periode . "</option>";
        }
        echo $data;
    }

    public function cari_pendaftar($tahun,$periode,$jenis)
    {
        $data['daftar'] = $this->psb->getDaftarCari($tahun,$periode,$jenis);
        $data['bayar'] = $this->psb->getBayar(); 
        $data['sch'] = $this->db->get('m_sekolah')->result_array();            
        $this->load->view('data_pendaftar/data_pendaftar_v_data', $data);       
    }
