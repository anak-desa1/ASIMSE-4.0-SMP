...
<script>
    $(document).ready(function() {
        $("#FormKI3").submit(function(e) {
            e.preventDefault();
            var id = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'tabel_ki3' ?>/" + id;
            $("#mapel").load(url);
        })
...
<script>
    $(document).ready(function() {
        $("#FormKI4").submit(function(e) {
            e.preventDefault();
            var id = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'tabel_ki4' ?>/" + id;
            $("#mapel").load(url);
        })
