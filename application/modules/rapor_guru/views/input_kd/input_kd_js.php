...
<script>
    $(document).ready(function() {
        $("#FormKdKI3").submit(function(e) {
            e.preventDefault();
            var id = $("#id_mapel").val();
            var target = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'data_kd_ki3' ?>/" + id + '/' + target;
            $("#kd").load(url);
        })
    ...
    $(document).ready(function() {
        $("#FormKdKI4").submit(function(e) {
            e.preventDefault();
            var id = $("#id_mapel").val();
            var target = $("#tingkat").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'data_kd_ki4' ?>/" + id + '/' + target;
            $("#kd").load(url);
        })
...
<script type="text/javascript">
    ...
    $('.btn-action').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>tambah/<?= $data['mapel_id'] ?>",
            data: "jenis=tambah",
    ...
    $('.btn-action-2').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>edit/" + id,
            data: "jenis=tambah",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-action').modal({
